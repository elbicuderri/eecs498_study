...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
      for j in range(num_test):
          dist = (x_train[i, :] - x_test[j, :]).pow(2).sum().sqrt()
          dists[i, j] = dist
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
      dist = (x_train[i, :] - x_test).pow(2).sum(dim=1).sqrt()
      dists[i, :] = dist
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def predict_labels(dists, y_train, k=1):
  ...
  # Replace "pass" statement with your code
  for j in range(num_test):
    min3 = torch.topk(dists[:, j], k, largest=False)
    min3s = y_train[min3.indices]
    m_d = dict()
    for v in torch.unique(min3s).tolist():
      freq = (min3s == v).sum()
      m_d[freq] = v
    y_pred[j] = m_d[max(m_d)]
